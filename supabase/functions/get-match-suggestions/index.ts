...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: currentUserProfileData, error: currentUserProfileError } = await supabaseClient
      .from('profiles')
      .select('profile_id')
    ...
    const { data: blockedRelations, error: blockedError } = await supabaseClient
      .from('blocked_users')
      .select('blocked_id')
    ...
    const { data: blockedByRelations, error: blockedByError } = await supabaseClient
      .from('blocked_users')
      .select('blocker_id')
      .eq('blocked_id', current_user_id);

    if (blockedByError) {
      console.error('Error fetching users who blocked current user:', blockedByError.message);
      return new Response(JSON.stringify({
        error: 'Error fetching blocked by users.'
      }), {
        headers: {
          ...corsHeaders,
          'Content-Type': 'application/json'
    ...
    blockedByRelations.forEach((block) => blockedUserIds.add(block.blocker_id));
    ...
    // Execute the query
    const { data: potentialSuggestions, error: profilesError } = await query
